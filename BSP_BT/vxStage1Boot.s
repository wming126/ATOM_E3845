...
--------------------
01h,26nov12,yjw  Fix cold reboot issue.(WIND00380360)
01g,31aug12,g_x  Jump to ROM_WARM_HIGH as the entry point of the second
                 stage. (WIND00332348)
01f,30jul12,g_x  Use .code16 instead of 0x66 prefix before call to avoid 
                 operand sizes of call and ret mismatch. (WIND00364527)
01e,28jun12,wyt  Add 0x66 operand size prefix (WIND00355697).
01d,16jan12,wyt  Skip bootA20On if > 1MB memory accessible (WIND00326992). 
01c,13jun11,j_l  Attempt to use local buffer for PXENV_GET_CACHED_INFO
                 first (Fix for WIND00281712).
01b,03feb11,j_l  multi-stage boot support (WIND00263692).
01a,14jul10,jb   Created
*/
...
reBootReset:
        ...
        je      1f
        mov     $RST_CNT_REG, %edx
        mov     $RST_CNT_SYS_RST, %eax
	outb	%al, %dx
        mov     $ (RST_CNT_SYS_RST | RST_CNT_CPU_RST | RST_CNT_FULL_RST), %eax
	outb	%al, %dx
rebootEntryHighHlt:
        hlt
