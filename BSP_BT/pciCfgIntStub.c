...
--------------------
01e,30jun14,jb   VXW6-8220 - PCI card IRQ connected via three or more PCI
                 bridges cannot be configured properly.
01d,19jul13,jjk  WIND00427114 - Fixing prevent defects
01c,25may12,j_z  configure interrupt line behind bridge, update
                 sysPciPirqShow based on device's actual information.
                 (WIND00346005)
01b,28jan11,rbc  Fix apigen errors.
01a,30sep10,rbc  initial creation based on itl_nehalem version 01y
*/
...
int sysPciFindMpEntry
    ...
    for (i = 0; i < (glbMpApicNioint + glbMpApicNloint); i++)
        ...
        if ((bus == sourceBusId) &&
            ...
            {
            /* adjustment for multiple IO APIC support, needs to sync with vxbIoApicIntr.c */
            ...
            if ((pMpApicInterruptTable->destApicId > glbMpApicIoBaseId) &&
                ...
                {
                irq += (ioApicRedNumEntries * (pMpApicInterruptTable->destApicId -
                       glbMpApicIoBaseId));
                }
             bcopy ((char *)pMpApicInterruptTable, (char *)pMpEntry, sizeof (MP_INTERRUPT));
             return irq;
            }
...
STATUS sysPciFindBridge
    ...
    if ((pciClass == ((PCI_CLASS_BRIDGE_CTLR << 8) | PCI_SUBCLASS_P2P_BRIDGE)) ||
        ...
        pList = (struct pciBridgeDevice *)hwMemAlloc(sizeof(struct pciBridgeDevice));
...
LOCAL STATUS sysPciPirqEnable2
    ...
    if (irq != ERROR)
        ...
        while (pList)
            ...
            /*
             * Ignore IRQ number for ISA devices (VXW6-8220)
             */

            if (irq != ERROR && irq >= IOAPIC_PIRQA_INT_LVL)
                {
                sysPciProgIrq (ctrl, bus, dev, func, arg, irq);
                return OK;
                }
