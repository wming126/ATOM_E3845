/* config.h - itl_atom configuration header */
...
--------------------
01q,26nov11,yjw  Update BSP version.(WIND00380360)
01p,08nov12,cfm  WIND00375416 - Added I8253 for non-vxprj build.
01o,05jun12,wyt  WIND00346005 - Change to GEN2 USB driver.
01n,25may12,j_l  WIND00351919 - Modify virtual wire mode DRV_TIMER_LOAPIC
                                comment
                 Adhere to coding standards more closely
01m,20apr12,jjk  WIND00329365 - Support for SMP
01l,27mar12,wyt  WIND00340694 - Change to new AHCI and ICH component.
01k,11jan12,jjk  WIND00203743 - Fix AuxClock failures
01j,17nov11,wyt  Add TOPCLIFF END driver.
01i,04oct11,jjk  WIND00308690 - Increment BSP revision
01h,22apr11,jjk  Multi-stage boot support.
01g,26apr11,sye  Update BSP version.
01f,11apr11,yjw  Fix incorrect SYS_MODEL when make bootrom.bin
                 (WIND00266072)
01e,07jan11,rbc  WIND00244749 - Add support for bundles.
...
#define BSP_VERSION "6.9.4.2"   /* vxWorks 6.9 BSP */
#define BSP_REV     "/7"    /* increment by whole numbers */
...
/* AD: the below macros are not include for bootrom */
...
#else
/* AD: the below macros are include for OS image */	
...
#undef INCLUDE_USR_BOOT_OP      /* user defined MP table */
...
/* define this to enable early formatted print statements. */
...
#define INCLUDE_EARLY_PRINT /* needed for ACPI debug printing */
...
/* Default boot line */
#define BOOT_MODE 0 /* 0:ahci 1:ftp server wming add */
#if !defined (CDF_OVERRIDE) && !defined (PRJ_BUILD)
#if BOOT_MODE /* 0: harddisk boot 1: net boot */
/* ftp server */
#define DEFAULT_BOOT_LINE \
    "gei(0,0)host:vxWorks h=192.168.10.103 e=192.168.10.145:ffffff00 f=0x00 u=target pw=password tn=DLINK o=gei"
#else
/* ahci */
#define DEFAULT_BOOT_LINE \
    "ata=0,1(0,0)host:/ata0:1/vxWorks h=192.168.10.103 e=192.168.10.145:ffffff00 f=0x00 u=target pw=password tn=DLINK o=gei"
#endif /* BOOT_MODE */
#endif /* !defined (CDF_OVERRIDE) && !defined (PRJ_BUILD) */
#undef BOOT_MODE
 ...
 * SYS_AP_LOOP_COUNT * SYS_AP_TIMEOUT gives the total time to wait for an
 * AP before giving up and moving on to the next application processor.
 */

#define SYS_AP_LOOP_COUNT   200000
#define SYS_AP_TIMEOUT      10
...
#undef  INCLUDE_VIRTUAL_WIRE_MODE       /* Virtual Wire Mode NOT Supported */
                                        /* on SMP */
#define DRV_TIMER_IA_HPET               /* SMP requires HPET */
#define INCLUDE_HPET_TIMESTAMP          /* SMP images use HPET for timestamp timer */
...
#undef SYS_MODEL
#define SYS_MODEL "Intel(R) Atom Processor SYMMETRIC IO"
#elif  defined (INCLUDE_VIRTUAL_WIRE_MODE)
#undef SYS_MODEL
#define SYS_MODEL "Intel(R) Atom Processor VIRTUAL WIRE"
#else
#define SYS_MODEL "Intel(R) Atom Processor"
...
#if !defined (CDF_OVERRIDE) && !defined (PRJ_BUILD)
#define  INCLUDE_DRV_STORAGE_AHCI   /* wming changes */
#endif /* !defined (CDF_OVERRIDE) && !defined (PRJ_BUILD) */
...
#define DRV_VXBEND_INTELTOPCLIFF
#define DRV_RESOURCE_INTELTOPCLIFF_IOH
...
#define DRV_TIMER_IA_TIMESTAMP  /* SMP will need timestamp for vxbDelay */
...
#define DRV_TIMER_I8253         /* add I8253 as default */
...
#ifdef INCLUDE_NORCO
#define SYSTEM_RAM_SIZE      (0x20000000)    /* Atom norko Platform minimum */
                                             /* 512MB system RAM     */
#else
#define SYSTEM_RAM_SIZE      (0x30000000)    /* Atom common Platform minimum */
                                             /* 768MB system RAM     */
#endif /* INCLUDE_NORKO */
...
#      define ROM_BASE_ADRS        (0x00108000)    /* base address of ROM */
#      define ROM_SIZE             (0x00200000)    /* size of ROM */
#   else /* GRUB_MULTIBOOT */
